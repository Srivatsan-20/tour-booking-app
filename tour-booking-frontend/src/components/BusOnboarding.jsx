  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setStatusMessage({
          type: "success",
          message: `🎉 Bus ${formData.registrationNumber} onboarded successfully! Bus ID: ${result.id}`
        });
        ...
        setStatusMessage({
          type: "danger",
          message: `Failed to onboard bus: ${errorData}`
        });
      }
    } catch (error) {
      console.error("Error onboarding bus:", error);
      setStatusMessage({
        type: "danger",
        ...
        <Col md={10} className="mx-auto">
          <Card className="shadow">
            <Card.Header className="bg-primary text-white">
              <h4 className="mb-0">🚌 Bus Onboarding</h4>
              <small>Add a new bus to your fleet</small>
            </Card.Header>
            <Card.Body>
